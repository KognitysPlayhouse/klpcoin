...
#if defined(NDEBUG)
# error "Klpcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5dcbfca9de9ed1570717afc71cd7cd8912576404deb323e6a6e64e258ae2a082");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Klpcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Klpcoin Signed Message:\n";
...
{
    // Klpcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840); // Klpcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Klpcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Klpcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Klpcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Klpcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x58ebba6e5b491cc7e7b1043843b0d97a26f2bfaf08721dad746a1bf31d86c7a0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KLP is the greatest";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044f8d9f39aee253935305db2c37eee249c65f978784117c3aa1f7dea9e156ed6c82bbd288703b3081fdd7fc609e8b3d344eb550776d216d8bf962326c889a60ae") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1588860797;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085143424;

        if (fTestNet)
        {
            block.nTime    = 1588860797;
            block.nNonce   = 386005954;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6c87ab4de91a17559144f1061c00cdbea23a67ec3ea5c304ef3ed5413b108c6a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xec, 0xc1, 0xf3, 0xaa }; // Klpcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KlpcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KlpcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KlpcoinMiner(CWallet *pwallet)
{
    printf("KlpcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("klpcoin-miner");
    ...
    try { loop {
        ...
        printf("Running KlpcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KlpcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KlpcoinMiner, pwallet));
}
